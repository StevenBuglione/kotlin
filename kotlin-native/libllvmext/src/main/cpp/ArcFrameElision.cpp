...
bool auditExclusiveFrameAliases(
    Value& value,
    AllocaInst& root,
    const DataLayout& dataLayout,
    const Function* enterFunction,
    ...
    unsigned& memsetCount) {
  if (!visited.insert(&value).second) return true;

  for (User* user : value.users()) {
    if (auto* cast = dyn_cast<BitCastInst>(user)) {
      if (GetUnderlyingObject(cast, dataLayout) != &root ||
          !auditExclusiveFrameAliases(*cast, root, dataLayout, enterFunction,
                                      leaveFunction, visited, enters, leaves,
                                      memsetCount)) {
        return false;
      }
      continue;
    ...
    if (auto* gep = dyn_cast<GetElementPtrInst>(user)) {
      ...
      if (!gep->isInBounds() || !gep->accumulateConstantOffset(dataLayout, offset) ||
          ...
          offset.getZExtValue() >= storageBytes ||
          GetUnderlyingObject(gep, dataLayout) != &root ||
          !auditExclusiveFrameAliases(*gep, root, dataLayout, enterFunction,
                                      leaveFunction, visited, enters, leaves,
                                      memsetCount)) {
        return false;
      }
    ...
    if (call == nullptr || call->arg_size() != 3 ||
        call->getNumOperandBundles() != 0 || call->isMustTailCall() ||
        !resolvesToFrameBase(call->getArgOperand(0), root, dataLayout)) {
      return false;
    }
    if (isDirectCallTo(*call, enterFunction)) {
...
bool findExclusiveCandidate(
  ...
  auto* root = dyn_cast<AllocaInst>(
      GetUnderlyingObject(candidateEnter.getArgOperand(0), dataLayout));
  ...
  unsigned memsetCount = 0;
  if (!auditExclusiveFrameAliases(*root, *root, dataLayout, enterFunction,
                                  leaveFunction, visited, enters, leaves,
                                  memsetCount) ||
      enters.size() != 1 || leaves.size() != 1 ||
      enters.front() != &candidateEnter || memsetCount > 1) {
    return false;
  }

  uint64_t leaveParameters = 0;
  uint64_t leaveCount = 0;
  CallInst* leave = leaves.front();
  ...
  result = FrameCandidate{
      root, nullptr, nullptr, &candidateEnter, leave, nullptr,
...
void collectExclusiveFrameStorageUsers(
    Value& value,
    DenseSet<Value*>& visited,
  ...
  for (User* user : value.users()) {
    if (auto* cast = dyn_cast<BitCastInst>(user)) {
      collectExclusiveFrameStorageUsers(*cast, visited, leaves, structural);
      structural.push_back(cast);
    } else if (auto* gep = dyn_cast<GetElementPtrInst>(user)) {
      collectExclusiveFrameStorageUsers(*gep, visited, leaves, structural);
...
void eraseExclusiveFrameStorage(FrameCandidate& candidate) {
  DenseSet<Value*> visited;
  SmallVector<Instruction*, 16> leaves;
  SmallVector<Instruction*, 16> structural;
  collectExclusiveFrameStorageUsers(
      *candidate.alloca, visited, leaves, structural);
  const DataLayout& dataLayout = candidate.alloca->getModule()->getDataLayout();
  for (Instruction* instruction : leaves) {
    bool verified = false;
    if (auto* memset = dyn_cast<MemSetInst>(instruction)) {
      verified = memset->getNumOperandBundles() == 0 &&
          isZeroMemsetOfRoot(*memset, *candidate.alloca, dataLayout);
    } else if (auto* intrinsic = dyn_cast<IntrinsicInst>(instruction)) {
      const Intrinsic::ID id = intrinsic->getIntrinsicID();
      verified = intrinsic->getNumOperandBundles() == 0 &&
          (id == Intrinsic::lifetime_start || id == Intrinsic::lifetime_end) &&
          isExactLifetimeMarker(*intrinsic, id, *candidate.alloca, dataLayout);
    }
    if (!verified) {
      report_fatal_error("unverified exclusive ARC frame storage cleanup instruction");
    }
...
int LLVMKotlinRemoveEmptyArcFrames(LLVMModuleRef moduleRef) {
  ...
  for (CallInst* enter : enterCalls) {
    ...
    if (findExclusiveCandidate(*enter, enterFunction, leaveFunction,
                               dataLayout, candidate) ||
        findCandidate(*enter, enterFunction, leaveFunction, setCurrentFrameFunction,
                      dataLayout, candidate)) {
  ...
  for (FrameCandidate& candidate : candidates) {
    ...
    if (candidate.eraseStorage) {
      eraseExclusiveFrameStorage(candidate);
    } else {
