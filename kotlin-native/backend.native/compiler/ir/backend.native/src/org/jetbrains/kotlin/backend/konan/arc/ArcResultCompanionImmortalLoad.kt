...
internal fun selectVerifiedResultCompanionImmortalLoads(
    ...
    if (constructor.parent !== companion || !constructor.isPrimary ||
        constructor.visibility != DescriptorVisibilities.PRIVATE || constructor.allParameters.isNotEmpty()
    ) return emptyList()
                    ...
                    body?.acceptVoid(object : IrElementVisitorVoid {
                        ...
                        override fun visitVariable(declaration: IrVariable) {
                            ...
                            if (call?.symbol?.owner === getter) {
                                val (reads, writes) = function.countUses(variable)
...
private fun IrSimpleFunction.countUses(variable: IrVariable): Pair<Int, Int> {
    var reads = 0
    var writes = 0
    body?.acceptVoid(object : IrElementVisitorVoid {
        override fun visitElement(element: IrElement) = element.acceptChildrenVoid(this)
        override fun visitGetValue(expression: IrGetValue) {
            if (expression.symbol.owner === variable) reads++
        }
        override fun visitSetValue(expression: IrSetValue) {
            if (expression.symbol.owner === variable) writes++
            expression.acceptChildrenVoid(this)
        }
    })
